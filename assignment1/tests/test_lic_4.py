...
def create_params(q_pts=0, quads=0, area1=0):
    ...
    """
    return Parameters(
        LENGTH1=0, RADIUS1=0, EPSILON=0, AREA1=area1,
        Q_PTS=q_pts, QUADS=quads, DIST=0, N_PTS=0, K_PTS=0,
        A_PTS=0, B_PTS=0, C_PTS=0, D_PTS=0, E_PTS=0,
        F_PTS=0, G_PTS=0, LENGTH2=0, RADIUS2=0, AREA2=0
    )

#unit test
def test_get_quadrant_axes():
    """Verify that points on axes follow priority rules (I > II > III > IV)"""
    from launch_interceptor_program.lic.geometry import get_quadrant
    assert get_quadrant((0, 0)) == 1   # Origo
    assert get_quadrant((-1, 0)) == 2  # Negativ X-axel
    assert get_quadrant((0, -1)) == 3  # Negativ Y-axel
    assert get_quadrant((1, -1)) == 4  # Standard Q4
...
def test_lic_4_sliding_window():
    ...
    """
    points = [
        (1, 1), (2, 2),   # Q1, Q1 (Början: bara 1 kvadrant)
        (1, 1), (-1, 1), (-1, -1) # Här triggas det: Q1, Q2, Q3 (3 kvadranter!)
    ]
    params = create_params(q_pts=3, quads=2)
    assert lic_4(points, params) is True